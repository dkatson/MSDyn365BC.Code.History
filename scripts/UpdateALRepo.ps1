...
foreach ($zip in $zips) {
    ...
    $ZipNameForTarget = (Split-Path -Path $RelativePath -Leaf).Replace('.zip', '').Replace('.Source', '')
    $ZipTargetPath = (Join-Path $ZipTargetPath $ZipNameForTarget)
    sz x $zip.FullName -o"$ZipTargetPath" -r -y | out-null
}
Get-ChildItem -Path $RepoPath -File -Filter *.xlf -Recurse | Remove-Item -Force
Get-ChildItem -Path (Join-Path $RepoPath 'BaseApp/Source') -Directory -Filter '*language*' | Remove-Item -Force -Recurse
